...
function listConfigurationProviders(req, res, next) {
  ...
  var type = req.params.type;
  // List providers
  barkley.findConfigurationProviders(idConfiguration, type, function(err, providers) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_ENTITY_ID_FOUND_MSG, idConfiguration)));
      }
...
function getConfigurationProvider(req, res, next) {
  ...
  var type = req.params.type;
  // Get resource
  barkley.getConfigurationProviderById(idConfiguration, idProvider, type, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
...
function attachConfigurationProvider(req, res, next) {
  ...
  var type = req.params.type;
  // Add filter
  barkley.attachConfigurationProvider(idConfiguration, idProvider, type, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_PROVIDER_CONFIGURATION_FOUND_MSG, idConfiguration, idProvider)));
      }
...
function detachConfigurationProvider(req, res, next) {
  ...
  var type = req.params.type;
  // Delete resource
  barkley.detachConfigurationProvider(idConfiguration, idProvider, type, function(err, resource) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_PROVIDER_CONFIGURATION_FOUND_MSG, idConfiguration, idProvider)));
      }
