...
/** @lends Repository */
{
  ...
  deleteResource: function(idProvider, idResource, cb) {
    ...
    try {
      ...
      for (var j = 0; j < resourceFilters.length; j++) {
        providerFilterresourceFilterCollectionCollection.remove(resourceFilters[j]);
      }
    } catch (err) {
      return cb(err);
    }
  ...
  */
  /**
  * Removes a configuration and any related filter-configuration configuration.
  * @param  {Object} idConfiguration Configuration identifier
  ...
  */
  deleteConfiguration: function(idConfiguration, cb) {
    ...
    try {
      ...
      // Remove filter-configuration configurations.
      var configurationConfigurations = filterConfigurationConfigurationsCollection.find({idConfiguration: idConfiguration});
      for (var i = 0; i < configurationConfigurations.length; i++) {
        filterConfigurationConfigurationsCollection.remove(configurationConfigurations[i]);
      }
    } catch(err) {
      return cb(err);
    }
