...
// TODO - Review all delete operations.

//
// Default barkley implementation based on memory backend.
//
...
/**
 * Barkley memory repository implementation. Only use for testing.
 * @class
 */
var Barkley =
/** @lends Repository */
{
  ...
  */
  ...
  /**
   * Callback for addProviderFilter.
   * @callback addProviderFilter_cb
   ...
   * @param {Object}  idFilter  Identifier
   * @param {String}  type  Filter's type
   * @param  {addProviderFilter_cb} cb Callback
   * @returns {void}
   */
  addProviderFilter: function(idProvider, idFilter, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
  ...
  /**
   * Callback for deleteProviderFilter.
   * @callback deleteProviderFilter_cb
   ...
   * @param {Object}  idFilter  Identifier
   * @param {String}  type  Filter's type
   * @param  {deleteProviderFilter_cb} cb Callback
   * @returns {void}
   */
  deleteProviderFilter: function(idProvider, idFilter, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
   ...
   * @returns {void}
   */
  findResourceFilters: function(idProvider, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
   ...
   * @returns {void}
   */
  getResourceFilterById: function(idProvider, idFilter, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
  ...
  /**
   * Callback for addResourceFilter.
   * @callback addResourceFilter_cb
   * @param {Error} err Error if provider or the filter does not exist or the
   * filter is already attached to the provider.
   * @param {Filter} Filter instance.
   */
   ...
   * @param {Object}  idFilter  Identifier
   * @param {String}  type  Filter's type
   * @param  {addResourceFilter_cb} cb Callback
   * @returns {void}
   */
  addResourceFilter: function(idProvider, idFilter, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
  ...
  /**
   * Callback for deleteResourceFilter.
   * @callback deleteResourceFilter_cb
   * @param {Error} err Error if provider does not exist or the
   * filter is not attached to the provider.
   * @param {Filter} Filter instance.
   */
   ...
   * @param {Object}  idFilter  Identifier
   * @param {String}  type  Filter's type
   * @param  {deleteResourceFilter_cb} cb Callback
   * @returns {void}
   */
  deleteResourceFilter: function(idProvider, idFilter, type, cb) {
    // Check provider exists
    var providerdb = providersCollection.findOne({id: idProvider});
    if (!providerdb) {
      return cb(new Errors.NoEntityFound());
