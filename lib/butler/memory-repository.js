...
var Butler = {
  ...
  */
  ...
  getConsumerFilterConfigByFilterAndConsumer: function(idConsumer, idFilter, cb) {
    var configdb = filterConsumerConfigurationsCollection.findOne({idFilter: idFilter, idConsumer: idConsumer});
    if (!configdb) {
      return cb(new Errors.NoEntityFound());
    }
  ...
  addConsumerFilterConfig: function(idConsumer, idFilter, props, cb) {
    ...
    if (filterConsumerConfigurationsCollection.findOne({idFilter: idFilter, idConsumer: idConsumer})) {
      return cb(new Errors.DuplicatedEntity());
    }
    // Check filter and consumer exists
    if (!filtersCollection.findOne({id: idFilter}) || !consumersCollection.find({id: idConsumer})) {
      return cb(new Errors.NoEntityFound());
    }
  ...
  getResourceById: function(idProvider, idResource, cb) {
    ...
    var resourcedb = resourcesCollection.findOne({idProvider: idProvider, id: idResource});
    if (!resourcedb) {
      return cb(new Errors.NoEntityFound());
    }

    // Cleanup lokie properties
    var resource = {
      id: resourcedb.id,
