...
function getProviderResource(req, res, next) {
  ...
  // Get resource
  butler.getResourceById(idProvider, idResource, function(err, resource) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
...
function listProviderFilters(req, res, next) {
  ...
  // List filters
  butler.findProviderFilters(idProvider, type, function(err, filters) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_ENTITY_ID_FOUND_MSG, idProvider)));
      }
...
function getProviderFilter(req, res, next) {
  ...
  var idFilter = req.params.idFilter;
  // Get resource
  butler.getProviderFilterById(idProvider, idFilter, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
...
function postProviderFilter(req, res, next) {
  ...
  // Add filter
  butler.addProviderFilter(idProvider, props, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_ENTITY_ID_FOUND_MSG, idProvider)));
      }
...
function deleteProviderFilter(req, res, next) {
  ...
  // Delete resource
  butler.deleteProviderFilter(idProvider, idFilter, function(err, resource) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_RESOURCE_FOUND_MSG, idProvider, idFilter)));
      }
      return next(err);
    }
    res.json(resource);
