...
function getProviderFilter(req, res, next) {
  ...
  var type = req.params.type;
  // Get resource
  butler.getProviderFilterById(idProvider, idFilter, type, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
...
function postProviderFilter(req, res, next) {
  ...
  var type = req.params.type;
  // Add filter
  butler.addProviderFilter(idProvider, idFilter, type, function(err, filter) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_FILTER_PROVIDER_FOUND_MSG, idProvider, idFilter)));
      }
...
function deleteProviderFilter(req, res, next) {
  ...
  var type = req.params.type;
  // Delete resource
  butler.deleteProviderFilter(idProvider, idFilter, type, function(err, resource) {
    if (err) {
      if (err instanceof Errors.NoEntityFound) {
        return next(new HttpError(404, util.format(NO_FILTER_PROVIDER_FOUND_MSG, idProvider, idFilter)));
      }
