...
/**
 * Configuration options and middlewares will be stores here.
 *
 * @type {Object}
 */
var configuration = {
...
function loadProviders(providers) {
  ...
  for (index = 0; index < num; index++ ) {
    ...
    if (!provider.context && !provider.host) {
      throw new InvalidConfiguration(InvalidConfiguration.INVALID_PROVIDER_MESSAGE, " Provider at index " + index + " has no 'host' neither 'context' property. At least one is required.");
    }
