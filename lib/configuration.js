...
/**
 * Load resources configuration loading its pre and postfilters modules.
 *
 * @private
 * @param  {Array<object>} resources Array with resources configuration.
 * @param  {boolean} providerIndex Index of the provider within array of providers.
 * @returns {Array<object>} Array with resources configuration plus a middleware
 * object referencing the filter's module.
 */
function loadResources(resources, providerIndex) {
  ...
  for (index = 0; index < num; index++ ) {
    ...
    // Check resource has required properties
    if (!resource.id) {
      throw new InvalidConfiguration(
        InvalidConfiguration.INVALID_RESOURCE_MESSAGE
        + " Resource at index " + index + " on"
        + " provider at index " + providerIndex + " has no 'id' property."
      );
    }
    if (!resource.context) {
      throw new InvalidConfiguration(
        InvalidConfiguration.INVALID_RESOURCE_MESSAGE
        + " Resource at index " + index + " on"
        + " provider at index " + providerIndex + " has no 'context' property."
      );
    }

    // Load resource's filters
    if (resource.prefilters) {
      prefilters = loadMiddlewareFilters(resource.prefilters);
    }
    if (resource.postfilters) {
      postfilters = loadMiddlewareFilters(resource.postfilters);
    }
...
function loadProviders(providers) {
  ...
  for (index = 0; index < num; index++ ) {
    ...
    if (provider.resources) {
      resources = loadResources(provider.resources, index);
    }
