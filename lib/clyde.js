...
function registerProviderMiddleware(clyde, proxy, provider) {
  ...
  provider.prefilters.forEach(function(filter) {
    ...
    clyde.use(provider.context, function(req, res, next) {
      log.debug({req: req}, "Before apply prefilter '%s' on provider '%s'...", filter.name, provider.name);
      next();
    });
  ...
  clyde.use(provider.context, function(req, res, next) {
    ...
    log.debug({req: req}, "Sending request to provider '%s'...", provider.name);
    ...
    proxy.on("proxyRes", function(proxyRes) {
      log.debug({res: proxyRes}, "Received response from provider '%s'...", provider.name);
    });
  ...
  provider.postfilters.forEach(function(filter) {
    ...
    clyde.use(provider.context, function(request, response, next) {
      log.debug({res: response}, "After apply postfilter '%s' on provider '%s'...", filter.name, provider.name);
      next();
    });
  });
}
...
module.exports.createProxyServer = function(config) {
  ...
  middlewares.prefilters.forEach(function(filter) {
    ...
    clyde.use(function(req, res, next) {
      log.debug({req: req}, "Before apply prefilter '%s'...", filter.name);
      next();
    });
  ...
  middlewares.postfilters.forEach(function(filter) {
    ...
    // This is only for log purposes
    clyde.use(function(req, res, next) {
      log.debug({res: res}, "After apply postfilter '%s'...", filter.name);
      next();
    });
  });
