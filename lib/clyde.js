...
var registerProviderMiddleware = function(clyde, proxy, provider) {
  ...
  provider.prefilters.forEach(function(filter) {
    ...
    clyde.use(function(req, res, next) {
      log.trace({req: req}, "Running prefilter '%s' on provider '%s'...", 
  ...
  clyde.use(provider.context, function(req, res, next) {
    ...
    log.trace({req: req}, "Proxy request to provider '%s'...", provider.name);
  ...
  provider.postfilters.forEach(function(filter) {
    // This is only for log purposes
    clyde.use(function(req, res, next) {
      log.trace({req: req}, "Running postfilter '%s' on provider '%s'...", 
        filter.name, provider.name);
      next();
    });
 ...
 * @return {middleware} Application instance, a request-response handler 
...
module.exports.createProxyServer = function(config) {
  ...
  middlewares.prefilters.forEach(function(filter) {
    // This is only for log purposes
    clyde.use(function(req, res, next) {
      log.trace({req: req}, "Running prefilter '%s'...", filter.name);
      next();
    });
  ...
  middlewares.postfilters.forEach(function(filter) {
    // This is only for log purposes
    clyde.use(function(req, res, next) {
      log.trace({req: req}, "Running postfilter '%s'...", filter.name);
      next();
    });
  ...
  clyde.use(function(err, req, res, next) {
    ...
    log.trace({req: req, err: err}, err.message);
